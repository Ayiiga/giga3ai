...
function ProfileEditor() {
  ...
  const authId = getCurrentUserId() || authUser?.id || '';
  ...
  const profileQuery = useQuery(api.users.getMyProfile, authId ? { authId } : 'skip');
  ...
  }, [profileQuery, authUser?.email, authUser?.image, authUser?.name, authUser?.phone]);
  ...
  }, [authId, photoStorageId, profile.email, profile.name, profile.phone, upsertProfile]);
...
export default function ProfileScreen() {
  ...
  const user = a0?.auth?.getUser?.();
  const convexUserEmail = user?.email || '';
  const subscription = useQuery(
    api.subscriptions.getActive,
    convexUserEmail ? { userId: convexUserEmail } : 'skip'
  );
